...
export default function Game({ onEnd }: GameProps) {
  ...
  const submitWord = useCallback(
    async (word: string) => {
      ...
      if (word === nextLLMWord) {
        onEnd([
          ...wordHistory,
          { llmWord: nextLLMWord, userWord: word, llmResponse: FinalResponse },
        ]);
        return;
      }
      if (!nextLLMWord) {
      ...
      setWordHistory([
        ...wordHistory,
        { llmWord: nextLLMWord, userWord: word, llmResponse: newLlmWord },
      ]);
  ...
  return (
    <div style={{ padding: '10px', maxWidth: '100vw' }}>
      {userWord === '' ? (
        <h2>Begin by entering a starting word</h2>
      ) : (
        <h3>Guesses: {wordHistory.length - 1}</h3>
      )}
      {userWord === '' && (
      ...
      <div>
        {showHint1 && (
          <HintLine hint="Find the midpoint between the two words, guess it exactly to make contact!" />
        )}
        {showHint2 && (
          <HintLine hint="Now find the *new* midpoint between your guess and the AI's" />
        )}
...
const WordRow = ({
  ...
  previousWords: { user: string; llm: string } | null;
...
const InputTextBox = ({
  ...
  return (
    <div
      style={{ width: '300px', margin: '0 auto', display: 'flex', gap: '5px' }}
    >
      <input
        type="text"
        maxLength={20}
